 ...
 * @param json 配置路径
 */
DB.prototype.filenameJson = function(json){
    var filename = path.join(path.dirname(__dirname).replace('app', ''), json);
    this.tabConf = JSON.parse(fs.readFileSync(path.normalize(filename)));
}
let db_ = null;
/**
 * 定义链接数据库
 * @param options 连接数据库信息
 *    
 *   {
 *      db_user: "dcm", 用户名
 *      db_pwd: "123456", 密码
 *      db_host: "192.168.1.175", ip地址
 *      db_port: 27017, 端口号
 *      db_name: "dcm" 库名称
 *   }
 *     
 */
const connet_db = function(options){
...
DB.prototype.getConnection = function (table_name) {
    ...
    if (!this.tabConf[table_name]) {
        logger.error('初始化 mongoose model 报错');
        return false;
    }
    ...
    if (!client) {
        ...
        //构建model
        client = db_.model(table_name, nodeSchema, table_name);
...
DB.prototype.save = function (table_name, fields, callback) {
    if (!fields) {
        if (callback) callback({msg: '保存数据没有传表数据报错'});
        return false;
    }
    var err_num = 0;
    for (var i in fields) {
    ...
    if (err_num > 0) {
        if (callback) callback({msg: '保存数据时候表明找不到报错'});
        return false;
    }
...
DB.prototype.update = function (table_name, conditions, update_fields, callback) {
    if (!update_fields || !conditions) {
        if (callback) callback({msg: '更新数据条件和更新的字段报错'});
        return;
    }
    var node_model = this.getConnection(table_name);
...
DB.prototype.updateData = function (table_name, conditions, update_fields, callback) {
    if (!update_fields || !conditions) {
        if (callback) callback({msg: '更新数据方法(带操作符的)条件和更新的字段报错'});
        return;
    }
    var node_model = this.getConnection(table_name);
 ...
 * 根据_id查询指定的数据
 * @param table_name 表名
 * @param _id 可以是字符串或 ObjectId 对象。  id: Object("1d23a1d321as31das32")
 * @param callback 回调方法
 */
